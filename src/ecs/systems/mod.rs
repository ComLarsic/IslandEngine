...
impl JsonConvert for JsonValue {
    fn as_vec2(&self) -> IslandResult<Vec2> {
        let x = self["x"]
            .as_f32()
            .ok_or(IslandError::new("No x-value provided"))?;
        let y = self["y"]
            .as_f32()
            .ok_or(IslandError::new("No u-value provided"))?;
    ...
    fn as_rect(&self) -> IslandResult<Rect> {
        ...
        let w = self["w"]
            .as_f32()
            .ok_or(IslandError::new("No w-value provided"))?;
        let h = self["h"]
            .as_f32()
            .ok_or(IslandError::new("No h-value provided"))?;
